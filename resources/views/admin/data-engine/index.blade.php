...
// Function to send report to WhatsApp
document.getElementById('sendToWhatsApp').addEventListener('click', function() {
    ...
    Swal.fire({
        title: 'Memuat...',
        text: 'Menyiapkan pengiriman ke WhatsApp',
    ...
    // Get operator's WhatsApp number from the system
    fetch('{{ route("admin.data-engine.get-operator-whatsapp") }}')
        ...
        .then(data => {
            if (data.success && data.whatsapp_number) {
                ...
                throw new Error('Nomor WhatsApp operator tidak ditemukan');
            }
        })
        .catch(error => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Gagal mengirim ke WhatsApp: ' + error.message
            });
        });
});
</script>
