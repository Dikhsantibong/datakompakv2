...
function getFormattedReport() {
    ...
    report += `Laporan Kesiapan KIT\n`;
    const unitSelect = document.querySelector('select[name="power_plant_id"]');
    const unitValue = unitSelect ? unitSelect.value : '';
    let unitName = unitSelect ? unitSelect.options[unitSelect.selectedIndex].text : 'Semua Unit';
    if (!unitValue) {
        unitName = 'Unit Pembangkit Kendari';
    }
    report += `${unitName}, ${formatDate(date)}\n`;
    report += `Pukul : ${time} Wita\n\n`;

    // Ambil semua card unit
    document.querySelectorAll('.bg-white.rounded-lg.shadow-sm.overflow-hidden.mb-6').forEach(section => {
        const title = section.querySelector('h3')?.textContent.trim() || '';
        report += `\n${title}\n`;
        // Info HOP/TMA/Inflow
        const infoElements = section.querySelectorAll('.flex.items-center.gap-2');
        infoElements.forEach(info => {
            ...
            if (label && value) report += `${label}: ${value} ${unit}\n`;
        });
        // Tabel mesin
        ...
        rows.forEach(row => {
            ...
            if (cells.length >= 9) {
                const mesin = cells[1].textContent.trim();
                const jam = cells[2].textContent.trim();
                const daya = cells[3].textContent.trim();
                const silm = cells[4].textContent.trim();
                const dmp = cells[5].textContent.trim();
                const beban = cells[6].textContent.trim();
                const status = cells[7].querySelector('span')?.textContent.trim() || cells[7].textContent.trim();
                const ket = cells[8].textContent.trim();
                report += `- ${mesin}:\n`;
                report += `  Jam: ${jam}\n`;
                report += `  Daya Terpasang: ${daya} kW\n`;
                report += `  SILM/SLO: ${silm}\n`;
                report += `  DMP: ${dmp}\n`;
